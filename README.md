...
============
 ...
 2. Run `./build` to create the local docker image `njd90/dhcpd6`.
...
on.
 ...
 2. Create `data/dhcpd6.conf` with a subnet clause for the specified
    network interface.  If you need assistance, you can run
    `docker run -it --rm njd90/dhcpd6 man dhcpd6.conf` for a description
    of the configuration file syntax.
 3. Run `docker run -it --rm --net=host -v "$(pwd)/data":/data njd90/dhcpd6 eth0`.
...
If a `/data` volume is not provided with a `dhcpd6.conf` inside it, the
